...
beta_sum = {}; sample_size = {}; scale = {}; idx = {}; se = {}
...
Omega = {}; sample_size_all = sample_size[0] + sample_size[1]
...
for blk in range(n_blk):
    ...
    ld_blk_all.append((w1 * ld_blk[0][blk] * np.mean(sample_size[0]) + (1 - w1) * ld_blk[1][blk] * np.mean(sample_size[1])) / np.mean(sample_size_all))
...
result_df.to_csv(output + '/coef_' + str(w1) + '.txt', sep='\t', index=False)
