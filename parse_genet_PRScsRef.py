...
def parse_ldblk(ldblk_dir, sst_dict, n_blk, eigen_rm, tmp_dir, sample_size):
    chrom = 22
    ...
    for blk in range(n_blk):
        ...
        if idx != []:
            ...
            sqrt_ld_blk.append(eigenvec[:, percent:].dot(np.diag(np.sqrt(eigenval[percent:]))).dot(eigenvec[:, percent:].T))
            inv_eigenval = eigenval
            inv_eigenval[inv_eigenval>0] = 1/inv_eigenval[inv_eigenval>0]
            inv_ld_blk.append(eigenvec[:, percent:].dot(np.diag(inv_eigenval[percent:])).dot(eigenvec[:, percent:].T))
