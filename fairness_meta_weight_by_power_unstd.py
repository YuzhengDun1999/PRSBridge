...
beta_sum = {}; sample_size = {}; scale = {}; idx = {}; se = {}; frq = {}; Pval = {}
...
Pval[0] = -np.log10(np.array(temp['P_x'])); Pval[1] = -np.log10(np.array(temp['P_y']))
...
for blk in range(1, n_blk):
    ...
    Pval[0] = np.append(Pval[0], -np.log10(np.array(temp['P_x']))); Pval[1] = np.append(Pval[1], -np.log10(np.array(temp['P_y'])))
...
ld_score_EUR = pd.read_table('/dcs04/nilanjan/data/ydun/MultiEthnic/' + Race[0] + '/ref_hapmap3/chr' + str(chr) + '/chr_' + str(chr) + '_merged.l2.ldscore', sep=r'\s+')
ld_score_AFR = pd.read_table('/dcs04/nilanjan/data/ydun/MultiEthnic/' + Race[1] + '/ref_hapmap3/chr' + str(chr) + '/chr_' + str(chr) + '_merged.l2.ldscore', sep=r'\s+')
ld_score = pd.merge(ld_score_EUR, ld_score_AFR, how='inner', on=['CHR', 'SNP', 'BP'])[['SNP', 'L2_x', 'L2_y']]
ld_score = pd.merge(result_df, ld_score, how='left', on=['SNP']); ld_score_0 = np.array(ld_score[['L2_x']])[:,0]; ld_score_1 = np.array(ld_score[['L2_y']])[:,0]
for i in range(n_pop):
    beta_sum[i] = beta_sum[i].astype(float)
    sample_size[i] = sample_size[i].astype(float)
    scale[i] = scale[i].astype(float)
Omega = {}; Sigma = {}
weight_0 = 1/ld_score_0 ** exponent
weight_1 = 1/ld_score_1 ** exponent
weight = w1 * weight_0 / (w1 * weight_0 + (1 - w1) * weight_1)
weight[weight_0 > weight_1] = w1
Omega[0] = weight / se[0]**2 / (weight/se[0]**2 + (1-weight)/se[1]**2)
Omega[1] = (1-weight) / se[1]**2 / (weight/se[0]**2 + (1-weight)/se[1]**2)
Sigma[0] = np.sqrt(2 * (frq[0] * (1-frq[0])))
Sigma[1] = np.sqrt(2 * (frq[1] * (1-frq[1])))
...
for blk in range(n_blk):
    ...
    M1_tmp = np.diag(Omega[0][idx_blk]/Sigma[0][idx_blk]).dot(ld_blk[0][blk]).dot(np.diag(Sigma[0][idx_blk])) + \
             np.diag(Omega[1][idx_blk]/Sigma[1][idx_blk]).dot(ld_blk[1][blk]).dot(np.diag(Sigma[1][idx_blk]))
    M2_tmp = np.diag(Omega[0][idx_blk]*scale[0][idx_blk]).dot(ld_blk[0][blk]).dot(np.diag(scale[0][idx_blk]*Omega[0][idx_blk]))/np.mean(sample_size[0]) + \
             np.diag(Omega[1][idx_blk]*scale[1][idx_blk]).dot(ld_blk[1][blk]).dot(np.diag(scale[1][idx_blk]*Omega[1][idx_blk]))/np.mean(sample_size[1])
...
var_y = (np.mean(scale[0]**2 * Sigma[0]**2) * np.mean(sample_size[0]) + np.mean(scale[1]**2 * Sigma[1]**2) * np.mean(sample_size[1])) / (np.mean(sample_size[0]) + np.mean(sample_size[1]))
if h2 > 0 and h2_se > 0:
    tau2 = h2 * var_y / beta_sum_all.shape[0] * math.gamma(1 / alpha) / math.gamma(3 / alpha) * np.mean(scale[0]**2*w1 + scale[1]**2*(1-w1))
