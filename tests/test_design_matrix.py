...
from bayesbridge.reg_coef_sampler.prs_cg_sampler import ConjugateGradientSampler
...
def test_Phi_precond_op():
    n = 1000
    test_sampler = ConjugateGradientSampler(5)
    prior_prec_sqrt = np.random.rand(n)
    ld = np.random.rand(n, n)
    obs_num = np.random.rand(n)
    precond_by = 'prior'
    beta_scaled_sd = np.random.rand(n)
    Phi_precond_op, precond_scale, Phi = \
        test_sampler.precondition_linear_system(
            prior_prec_sqrt, ld, obs_num, precond_by, beta_scaled_sd
        )
    test_array = np.random.rand(n)

    assert np.allclose(Phi_precond_op.matvec(test_array),
                       Phi.dot(test_array))