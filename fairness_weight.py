...
parser.add_argument('--w1', dest='w1', type=float, help='weight of EUR')
...
beta_sum_all = beta_sum[0] * np.mean(sample_size[0]) * w1 + beta_sum[1] * np.mean(sample_size[1]) * (1-w1)
...
for blk in range(n_blk):
    ...
    ld_blk_all.append((ld_blk[0][blk] * np.mean(sample_size[0]) * w1 + ld_blk[1][blk] * np.mean(sample_size[1]) * (1-w1)))
sample_size_weight = np.mean(sample_size[0]) * w1 + np.mean(sample_size[1]) * (1-w1)
beta_index = 0
...
for blk in range(n_blk):
    ...
    start_idx = max(sum(eigenval < 0.01 * sample_size_weight), percent)#previous results are 0.05
...
result_df['BETA'] = coef
result_df.to_csv(output + '/coef_standardized_' + str(w1) + '.txt', sep='\t', index=False)

result_df['BETA'] = coef * scale[0]
result_df.to_csv(''.join(
    [output, '/coef_EUR_weight_', str(w1),
     '.txt']),
                 sep='\t', index=False)
result_df['BETA'] = coef * scale[1]
result_df.to_csv(''.join(
    [output, '/coef_AFR_percent_', str(w1),
     '.txt']),
                 sep='\t', index=False)