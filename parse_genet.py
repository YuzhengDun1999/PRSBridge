...
def parse_ref(ref_file, n_blk, chrom):
    ...
    for blk in range(1,n_blk+1):
        snplist = []
        ref_file1 = ref_file + '/block' + str(blk) + '.bim'
        #ref_file1 = ref_file + '/chr_22_merged.bim'
        with open(ref_file1) as ff:
            for line in ff:
                ll = (line.strip()).split()
                if int(ll[0]) == chrom:
...
def parse_sumstats(ref_dict, vld_dict, sst_file):
...
def parse_ldblk(ldblk_dir, sst_dict, snp_blk, n_blk, eigen_rm, tmp_dir, sample_size):

    ld_blk = [np.load(ldblk_dir+'/block'+str(blk)+'.npy') for blk in range(1,n_blk+1)]
    #ld_blk = [np.load('../chr22/chr_22_merged.npy')]
    ...
    #hdf_chr = h5py.File('../PRScs/ref/ldblk_1kg_eur/ldblk_1kg_chr22.hdf5', 'r')
    #n_blk = len(hdf_chr)
    ...
    for blk in range(n_blk):
        ...
        if idx != []:
            ...
            ldsc.append(np.sum(ld_blk[blk]**2, axis=0))
            eigenval, eigenvec = np.linalg.eigh(ld_blk[blk])
            eigenval[eigenval<0] = 0
            percent = int(np.floor(eigenval.shape[0]*eigen_rm))
            ld_blk[blk] = eigenvec[:, percent:].dot(np.diag(eigenval[percent:])).dot(eigenvec[:, percent:].T)
            sqrt_ld_blk[blk] = eigenvec[:, percent:].dot(np.diag(np.sqrt(eigenval[percent:]))).dot(eigenvec[:, percent:].T)
            inv_eigenval = eigenval
            inv_eigenval[inv_eigenval>0] = 1/inv_eigenval[inv_eigenval>0]
            inv_ld_blk[blk] = eigenvec[:, percent:].dot(np.diag(inv_eigenval[percent:])).dot(eigenvec[:, percent:].T)
    ...
    ldsc_array = ldsc[0]
    for i in range(1,len(ldsc)):
        ldsc_array = np.append(ldsc_array, ldsc[i])

    np.savetxt(tmp_dir + "/coef.csv", np.array(sst_dict['BETA']), delimiter=",")
    np.savetxt(tmp_dir + "/ldsc.csv", ldsc_array, delimiter=",")

    command = 'Rscript'
    path2script = 'ldsc.R'
    args = [tmp_dir + '/coef.csv', tmp_dir + '/ldsc.csv', str(sample_size)]
    # Build subprocess command
    cmd = [command, path2script] + args
    # check_output will run the command and store to result
    x = subprocess.check_output(cmd, universal_newlines=True)
    h2 = float(x.strip().split()[1])
    h2_se = float(x.strip().split()[2])

    return ld_blk, sqrt_ld_blk, inv_ld_blk, blk_size, h2, h2_se
